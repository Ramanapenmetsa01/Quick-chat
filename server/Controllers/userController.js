...
export const signup = async (req, res) => {
    const { fullName, email, password, bio, publicKey, encryptedPrivateKey, salt, iv } = req.body

    try {
        if (!fullName || !email || !password || !bio || !publicKey || !encryptedPrivateKey || !salt || !iv) {
        ...
        const newUser = await User.create({
            fullName, email, password: hashedPassword, bio, publicKey, encryptedPrivateKey, salt, iv
        })
        const token = generateToken(newUser._id)

        res.json({
            success: true, token, message: "Account created succesfully", userData: {
                _id: newUser._id,
