      ...
      <form  className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        ...
        <button
          ...
          onClick={() => {
            ...
            if (currState === "Sign up" && !isNext) {
              if (!inputDetails.fullName || !inputDetails.email || !inputDetails.password ||!isChecked) {
            ...
            login(currState=="Sign up"?"signup":"login",inputDetails)

          
          }}
          className='py-3 bg-gradient-to-r from-purple-400 to-violet-600 text-white rounded-md cursor-pointer'>
          {currState === "Sign up" ? !isNext ? "Next" : "Create Account" : "Login"}
        </button>
        ...
        <div className='flex items-center gap-2 text-sm text-gray-5000'>
          ...
          <div className='flex flex-col gap-2'>
            {currState==='Sign up'?(
              <p className='text-sm text-gray-600'>Already have an account? <span onClick={()=>{setCurrState("Login"); setIsNext(false)}}className='font-medium text-violet-500 cursor pointer'>Login</span></p>
            ):(
